...
\prod_{g=1}^{G} \prod_{i=1}^{n_g} p(y_{gi} \mid \theta_g)
\;\cdot\;
\prod_{g=1}^{G} p(\theta_g \mid \phi)
\;\cdot\;
p(\phi)
$$
...
| term | what it is |
|---|---|
| $\prod_g \prod_i p(y_{gi} \mid \theta_g)$ | **likelihood** — trial $i$ of participant $g$ |
...
In ArviZ: `az.loo(idata)`. Note the attribute is `.p`, though it *prints* as
`p_loo`.
:::
...
$\log\tau \approx -0.3$; the non-centered one carries on to $-6.9$.

| weak likelihood | centered | non-centered |
|---|---|---|
| divergences | **346** | 13 |
| reaches $\log\tau$ | $-0.3$ | $\mathbf{-6.9}$ |
...
Centered went from 346 divergences to **zero**. Non-centered went from 13 to
2 — but look at its panel: $z_1$ and $\log\tau$ are now on a hard **diagonal
ridge**. To keep $\theta_g$ where the data wants it while $\tau$ grows, $z_g$
must shrink. That is the inverted funnel.
...
because **different parameters are informed by different amounts of data**,
...
- **Boundary $a$** is identified mostly through the speed–accuracy
  relationship, so it needs *variation* rather than volume — far fewer
